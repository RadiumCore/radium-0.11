...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nFees)
{
    ...
    else if(pindexPrev->nHeight+1 >= 1392760 && pindexPrev->nHeight+1 < DEV_FUND_BLOCK_HEIGHT)
    {
        nSubsidy = 0.25 * COIN; // Third and final reward drop 18 months from the average fee fork.
    }
    else if(pindexPrev->nHeight+1 >= DEV_FUND_BLOCK_HEIGHT && pindexPrev->nHeight+1 < DEV_FUND_BLOCK_HEIGHT + nYear)
    {
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        {
            int nSpendDepth;
            // this is nHeight +1 because its using pindexPrev hight, so +1 brings it to this block height
            if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, nStakeMinConfirmations(pindexPrev->nHeight +1) - 1, nSpendDepth))
            {
